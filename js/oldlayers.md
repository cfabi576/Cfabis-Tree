...
31: {
            ...
            addLayer("uf", {
             ...
             autoUpgrade() {if (hasUpgrade("jp", 25))  return true; else return false},
                ...
                gainMult() { // Calculate the multiplier for main currency from bonuses
                    ...
                    if (hasUpgrade('uf', 61)) mult = mult.times(3.5)
    ...
    if (hasUpgrade('uf', 106)) mult = mult.times(5000)
            if (hasUpgrade('uf', 155)) mult = mult.times(upgradeEffect('uf', 155))
                 if (hasUpgrade('jp', 36)) mult = mult.times(upgradeEffect('jp', 36))
                      if (hasUpgrade('jp', 15)) mult = mult.pow(upgradeEffect('gb', 31))
                               if (hasUpgrade('jp', 34)) mult = mult.pow(1.1)
        ...
        if (layers[resettingLayer].row > this.row) layerDataReset("uf", keep)
			...
			currencyDisplayName: "points",
                    ...
                    61:{
                        ...
                        description: "x3.5 nice boost of UF!",
   ...
   if	(hasUpgrade('uf', 83)) limit = limit+20
    if	(hasUpgrade('uf', 87)) limit = limit+20
     if (hasUpgrade('uf', 145)) limit = limit+30
      if (hasUpgrade('jp', 33)) limit = limit+70
                  return purchaseLimit = limit
                    ...
                    cost(x) {
                        ...
                        return new Decimal(100).mul(Decimal.pow(1.25, x)).mul(Decimal.mul(x , Decimal.pow(exp2 , x))).floor()
...
addLayer("jp", {
  ...
  keepData() { return ["keys"] },
   ...
   onReset(layer) {
        // 👇 evita que keys se borre o se corrompa en resets
        if (layer == this.layer) return
        if (!player.jp) player.jp = {}
        if (!(player.jp.keys instanceof Decimal)) player.jp.keys = new Decimal(0)
    },


  fixKeys() {
    // Repara valores inválidos en player.jp.keys
    if (!player.jp) player.jp = {}
    if (player.jp.keys == undefined || player.jp.keys == null) player.jp.keys = new Decimal(0)
    if (!(player.jp.keys instanceof Decimal)) {
        // intenta convertir lo que haya a Decimal
        try {
            player.jp.keys = new Decimal(player.jp.keys)
            if (player.jp.keys.isNaN()) throw "bad"
        } catch {
            player.jp.keys = new Decimal(0)
        }
    }
},
    ...
    upgrades: {
        ...
        unlocked() { return hasUpgrade("jp", 24) },
          ...
          31: {
            title: "#212: Placid 1",
            description: "I feel very placid, so get a ^1.01 point gain boost.",
            cost: new Decimal(1),
        unlocked() { return hasUpgrade("jp", 25) },
        },
              32: {
            title: "#213: Placid 2",
            description: "Loop^2! Unlock the bonus side layer. Only for fun!, Not really a upgrade, its more a feature...",
            cost: new Decimal(1),
        unlocked() { return hasUpgrade("jp", 31) },
        },
           33: {
            title: "#214: Placid 3",
            description: "+70 #149's cap?",
            cost: new Decimal(1),
        unlocked() { return hasUpgrade("jp", 32) },
        },
         34: {
            title: "#215: Placid 4",
            description: "Do it two times! Be happy! you can relax here. ^1.1 UF?????",
            cost: new Decimal(2),
        unlocked() { return hasUpgrade("jp", 33) },
        },
      35: {
    title: "#216: Placid 5",
    description: "Night is good! If your local time is between 6:00 PM and 11:30 PM, get a ×15 point boost.",
    cost: new Decimal(1),
    unlocked() { return hasUpgrade("jp", 34) },

    // 🔮 Efecto de tiempo local
    effect() {
        const now = new Date();
        const hours = now.getHours();
        const minutes = now.getMinutes();
        const time = hours + minutes / 60; // hora decimal

        // entre 18.0 (6 PM) y 23.5 (11:30 PM)
        if (time >= 18 && time <= 23.5) return new Decimal(15);
        return new Decimal(1);
    },

    effectDisplay() {
        const eff = this.effect();
        if (eff.gt(1)) return `Active! ×${format(eff)} boost`;
        else return `Inactive — it's not night time`;
    },
},
 36: {
    title: "#217: Placid 6",
    description: "UF in the morning! If your local time is between 6:00 AM and 4:30 PM, get a ×7 UF boost.",
    cost: new Decimal(1),
    ...
    effect() {
        ...
        // entre 18.0 (6 PM) y 23.5 (11:30 PM)
        if (time >= 6 && time <= 16.5) return new Decimal(7);
        return new Decimal(1);
    },
    ...
    effectDisplay() {
        const eff = this.effect();
        if (eff.gt(1)) return `Active! ×${format(eff)} boost`;
        else return `Inactive — it's not morning time :/`;
    },
},
    
   37: {
    title: "#218: Placid 7",
    description: "Rush please! Boost changes from ×1 to ×5 depending on the seconds of your local time. (0s = ×1, 59s = ×5) (DOES NOT WORK AT ALL)",
    cost: new Decimal(1),
    unlocked() { return hasUpgrade("jp", 36) },

    effect() {
        const now = new Date();
        const seconds = now.getSeconds(); // segundos locales (0–59)
        // Escala lineal de 1x a 5x según los segundos
        const mult = 1 + (seconds / 59) * 4; // 0s→1, 59s→5
        return new Decimal(mult);
    },

    effectDisplay() {
        return `Currently ×${format(this.effect())} points (based on seconds: ${new Date().getSeconds()})`;
    },
},
  41: {
            title: "#219: Press A Key 1",
            description: "New Stats Rise Up!  Unlock a new tab + A new currency called..... Keys!!!!!!!",
            cost: new Decimal(1),
        unlocked() { return hasUpgrade("jp", 37) },
        },
          42: {
            title: "#220: Press A Key 2",
            description: "Those will be too long... x2 Key Gain",
            cost: new Decimal(0.20),
        unlocked() { return hasUpgrade("jp", 41) },
           currencyInternalName: "keys",
                        currencyDisplayName: "Keys",
                        currencyLayer: "jp",
        },
          43: {
            title: "#221: Press A Key 3",
            description: "Upgrades n Keys collaboration?! Upgrade after 225 boosts Keys, a lot.",
        ...
        let extra = Math.max(totalUpgs - 225, 0)

    
        return Decimal.pow(1.05, extra)
    },
    ...
    effectDisplay() {
      ...
      44: {
            title: "#222: Press A Key 4",
            description: "Happylike 2 boosts Keys omg",
        ...
        unlocked() { return hasUpgrade("jp", 41) },
           ...
           45: {
            title: "#223: Press A Key 5",
            description: "Keys keys. Keys boosts itself",
            cost: new Decimal(5),
                 tooltip() {
                    
                return hasUpgrade("jp", 52) ? "log10(Keys)^1.25+1" : "log10(Keys)+1"
            },
        unlocked() { return hasUpgrade("jp", 41) },
         effect() {
            let hi = new Decimal(1)
            if (hasUpgrade("jp", 52)) hi = hi.add(0.25)
                 if (hasUpgrade("jp", 53)) hi = hi.add(0.1)
            
        return player.jp.keys.log10().pow(hi).add(1)
     
    },

    effectDisplay() {
        return `Currently ×${format(this.effect())} points.`;
    },
           currencyInternalName: "keys",
                        currencyDisplayName: "Keys",
                        currencyLayer: "jp",
     ...
     46: {
        ...
        unlocked() { return hasUpgrade("jp", 45) },
           currencyInternalName: "keys",
             ...
             47: {
            title: "#225: Press A Key 7",
            description: "Skyrocket the keys! ^1.03 keys",
        ...
        unlocked() { return hasUpgrade("jp", 46) },
           currencyInternalName: "keys",
               ...
               51: {
            title: "#226: Press A Key 8",
            description: "Keys free break! x3.14 Keys!",
        ...
        unlocked() { return hasUpgrade("jp", 47) },
          ...
          52: {
            title: "#227: Press A Key 9",
            description: "Keybot! Keys Boost Keys V2! add a +^0.25 effect pow on #223",
        ...
        unlocked() { return hasUpgrade("jp", 51) },
           currencyInternalName: "keys",
            ...
            53: {
            title: "#228: Press A Key 10",
            description: "Upgraded Keybot! Keys Boost Keys V3! add a +^0.10 effect pow on #223....",
        ...
        unlocked() { return hasUpgrade("jp", 52) },
           ...
           54: {
            title: "#229: Press A Key 11",
            description: "Unlocks the.... Class Indicator! (Useless)- wait it doesnt exist. so Unlock the uhh... x4 Keys???",
            cost: new Decimal(1000),
        unlocked() { return hasUpgrade("jp", 53) },
         ...
         55: {
            title: "#230: Press A Key 12",
            description: "Key after Key reference omg he said x1.66 keys",
        ...
        56: {
            title: "#231: Press A Key 13",
            description: "Donate Me Keys 😢 Keys major. +1 key gain",
        ...
        unlocked() { return hasUpgrade("jp", 55) },
           ...
           57: {
            title: "#232: Press A Key 14",
            description: "End of the keys vrrom vroom skibidi x4 or x3 we donmt know skibidi rizzler tung tung tung tung sahur tralalero sprunki brainrot r/youngpeople tatata goo goo gaa gaa klasky csupo g major 40 295 slb cfabi pyrqamixed=!?=#??",
   ...
   61: {
            title: "#233: Tap To Move 1",
            description: "Have you got more than 20000 keys? Keys Boosts Points slightly.",
            cost: new Decimal(20000),
        unlocked() { return hasUpgrade("jp", 57) },
        effect() {
      return player.jp.keys.log10().pow(1.2).add(1)
    },
  ...
  62: {
            title: "#234: Tap To Move 2",
            description: "Keys LOVES points. Points boosts keys.",
            cost: new Decimal(30000),
        unlocked() { return hasUpgrade("jp", 61) },
        effect() {
      return player.points.log10().log10().pow(1.2).add(1)
    },
    ...
    63: {
            title: "#235: Tap To Move 3",
            description: "Side Layer means... useful! Unlock Chemicals.",
        ...
        unlocked() { return hasUpgrade("jp", 62) },
         ...
         64: {
            title: "#236: Tap To Move 4",
            description: "Secondary Jumpify Challenge! ",
            cost: new Decimal(1),
        unlocked() { return hasUpgrade("jp", 62) },
...
update(diff) {
    ...
    if (hasUpgrade("jp", 41)) {
        let hi = new Decimal(0.01)
        if (hasUpgrade('jp', 42)) hi = hi.times(2)
        if (hasUpgrade('jp', 43)) hi = hi.times(upgradeEffect('jp', 43))
        if (hasUpgrade('jp', 44)) hi = hi.times(upgradeEffect('uf', 112))
        if (hasUpgrade('jp', 45)) hi = hi.times(upgradeEffect('jp', 45))
        if (hasUpgrade('jp', 62)) hi = hi.times(upgradeEffect('jp', 62))
        if (hasUpgrade('jp', 46)) hi = hi.times(5)
        if (hasUpgrade('jp', 51)) hi = hi.times(3.14)
        if (hasUpgrade('jp', 54)) hi = hi.times(4)
        if (hasUpgrade('jp', 56)) hi = hi.add(1)
        if (hasUpgrade('jp', 55)) hi = hi.times(1.66)
        if (hasUpgrade('jp', 47)) hi = hi.pow(1.03)

        const gain = hi.times(diff)
        player.jp.keys = player.jp.keys.add(gain)
    }
},
...
12: {
    name: "Point Mass Down.",
    challengeDescription() {
        let comps = challengeCompletions("jp", 12)
        let debuff = 0.20 - comps * 0.01 // cada completado reduce 5%
        if (debuff < 0.05) debuff = 0.05 // límite mínimo
        return `Your point production is raised to ^${format(debuff)}.`
    },
    goal() {
        let comps = Math.max(1, challengeCompletions("jp", 12))
        return new Decimal("1e750").times(Decimal.pow(comps, 5))
    },
    rewardDescription() {
        let comps = challengeCompletions("jp", 12)
        let boost = new Decimal(1).add(comps*0.1)
        return `Raise Point production by ^${format(boost)}`
    },
    rewardEffect() {
        let comps = challengeCompletions("jp", 12)
        return new Decimal(1).add(comps*0.1)
    },
    rewardDisplay() {
        return `^${format(this.rewardEffect())}`
    },
    canComplete() { 
        return player.points.gte(tmp.jp.challenges[12].goal)
    },
    completionLimit: 15,
    unlocked() { 
        return hasUpgrade("jp", 64)
    },
},
    ...
    tabFormat: {
        "Upgrades": {
            content: [
                ...
                ["display-text", () => `<h3>Earn Jump Points to progress, to unlock some OP thing!</h3>`],
                  ["display-text", () => `<h3>You have <span style="color:#ffffff">${format(player.jp.keys)}</span> Keys (Spoiler?)</h3>`],
                "blank",
                "upgrades",
            ],
        },
        ...
        "Challenges": {
          ...
          "Keys": {
            unlocked() { return hasUpgrade("jp", 41) }, // 🔒 Pestaña entera oculta hasta upgrade 11
            content: [
                "main-display",
                "prestige-button",
                 "blank",
         ["display-text", () => `<h3>You have <span style="color:#ffffff">${format(player.jp.keys)}</span> Keys</h3>`],
              
            ["display-text", () => `<h3>You can use those to get Press A Key Upgrades, or even Walk A Slope!</h3>`],
                "blank",
   ...
   22: {
            ...
            25: {
              ...
              27: {
                name: "Keys!",
                done() { return hasUpgrade("jp", 41) },
                tooltip: "Unlock Keys",
            },
                31: {
                name: "All the keys of a Keyboard",
                done() { return player.jp.keys.gte(108) },
                tooltip: "Get 108 Keys",
            },
             32: {
                name: "Chemicals!",
                done() { return hasUpgrade("jp", 63) },
                tooltip: "Unlock Chemicals",
            },
        },
        	tabFormat: [
			"blank", 
   ...
   addLayer("b", {
    ...
    startData() { return {
        unlocked: true, 
        points: new Decimal(0), // Alpha
    }},
    color: "#00ff40",
    row: "side",
    layerShown() { return hasUpgrade("jp", 31) }, 
    tooltip() { 
        return ("Bonus")
    },
    ...
    update(diff) {
        ...
        if (hasUpgrade("b", 11)) {
            let gain = new Decimal(0.01).times(diff); // 0.01 por segundo
            player.b.points = player.b.points.add(gain);
        }
    },

    tabFormat: [
        ["display-text", () => `<h3>You have <span style="color:#00ff40">${format(player.b.points)}</span> Alpha</h3>`],
        ["display-text", () => `<small>This layer is <b>bonus</b>. This is side progression and does not give boosts to main progress.</small>`],
        "blank",
        "upgrades",
    ],
    ...
    upgrades: {
        11: {
            title: "Bonus Upgrade 1",
            description: "Start generating Alpha at 0.01/s.",
            cost: new Decimal(0),
            unlocked() { return true },
        },
           12: {
            title: "Bonus Upgrade 2",
            description: "Joke Joke Joke x0.99999999999999999999999999999999999 alpha gain.",
...
addLayer("c", {
    ...
    color: "#0b004bff",
    row: "side",
    layerShown() { return hasUpgrade("jp", 63) }, 
    tooltip() { 
        return ("Bonus")
    },
    ...
    update(diff) {
        ...
        // Genera Alpha pasivamente si tienes la mejora 11
        if (hasUpgrade("c", 11)) {
            let atoms = new Decimal(0.001)
            if (hasUpgrade("c", 12)) atoms = atoms.times(2)
                   if (hasUpgrade("c", 12)) atoms = atoms.times(2.5)
            let gain = new Decimal(0.001).times(diff); // 0.01 por segundo
            player.c.points = player.c.points.add(gain);
        }
    },

    tabFormat: [
        ["display-text", () => `<h3>You have <span style="color:#0b004bff">${format(player.c.points)}</span> Atoms</h3>`],
    ...
    upgrades: {
        11: {
            title: "#C1: Hydrogen ",
            description: "Start generating Atoms at 0.001/s.",
            cost: new Decimal(0),
            unlocked() { return true },
        },
        12: {
            title: "#C2: Helium ",
            description: "x2 Atoms.",
            cost: new Decimal(0.01),
            unlocked() { return true },
        },
           13: {
            title: "#C3: Lithium ",
            description: "x2.5 Atoms.",
